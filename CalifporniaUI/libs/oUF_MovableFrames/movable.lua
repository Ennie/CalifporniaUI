...
do
	...
	function frame:VARIABLES_LOADED()
		...
		if Califpornia.CFG.unitframes.movable_perchar then
			if not CalifporniaDB.MovableFrames[Califpornia.myrealm] then
				CalifporniaDB.MovableFrames[Califpornia.myrealm] = {}
			end
			if not CalifporniaDB.MovableFrames[Califpornia.myrealm][Califpornia.myname] then
				CalifporniaDB.MovableFrames[Califpornia.myrealm][Califpornia.myname] = {}
			end
			_DB = CalifporniaDB.MovableFrames[Califpornia.myrealm][Califpornia.myname] or {}
			CalifporniaDB.MovableFrames = _DB
		else
			_DB = CalifporniaDB.MovableFrames or {}
			CalifporniaDB.MovableFrames = _DB
		end
...
SlashCmdList['OUF_MOVABLEFRAMES'] = function(inp)
	...
	if(inp == 'reset') then
		...
		print('Positions set to default')
	else
		if(not _LOCK) then
			for k, obj in next, oUF.objects do
				local style, identifier, isHeader = getObjectInformation(obj)
