...
if CalifporniaCFG["datatext"].gold and Califpornia.CFG["datatext"].gold > 0 then
	...
	local Text  =Califpornia.Panels.CreateDataText(Califpornia.CFG["datatext"].gold)
	...
	local function OnEvent(self, event)
		...
		self:SetAllPoints(Text)
 		...
 		if (CalifporniaDB.Gold == nil) then CalifporniaDB.Gold = {} end
		if (CalifporniaDB.Gold[Califpornia.myrealm]==nil) then CalifporniaDB.Gold[Califpornia.myrealm]={} end
		CalifporniaDB.Gold[Califpornia.myrealm][Califpornia.myname] = GetMoney();
		...
		self:SetScript("OnEnter", function()
			if not InCombatLockdown() then
				...
				local thisRealmList = CalifporniaDB.Gold[Califpornia.myrealm]
				for k,v in pairs(thisRealmList) do
					GameTooltip:AddDoubleLine(k, FormatTooltipMoney(v), 1, 1, 1, 1, 1, 1)
					totalGold=totalGold+v;
				end 
				GameTooltip:AddLine' '
				GameTooltip:AddLine(Califpornia.myrealm..": ")
