...
if CalifporniaCFG["datatext"].system and Califpornia.CFG["datatext"].system > 0 then
	...
	local Text  =Califpornia.Panels.CreateDataText(Califpornia.CFG["datatext"].system)
	...
	Stat:SetScript("OnEnter", function(self)
		if not InCombatLockdown() then
			...
			GameTooltip:AddDoubleLine("Realm Latency: ",latencyHome..'ms',0, 0.6, 1, 1, 1, 1)
			GameTooltip:AddDoubleLine("World Latency: ",latencyWorld..'ms',0, 0.6, 1, 1, 1, 1)
			GameTooltip:AddDoubleLine("Bandwidth: ",format("%s ".."Mbps",Round(bandwidth, 2)),0, 0.6, 1, 1, 1, 1)
			GameTooltip:AddDoubleLine("Download: ",format("%s%%", floor(GetDownloadedPercentage()*100+0.5)),0, 0.6, 1, 1, 1, 1)
