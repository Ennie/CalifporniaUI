...
if CalifporniaCFG["datatext"].friends and Califpornia.CFG["datatext"].friends > 0 then
	...
	local Text  =Califpornia.Panels.CreateDataText(Califpornia.CFG["datatext"].friends)
	...
	local function Update(self, event)
			...
			Text:SetText(FRIENDS..": ".."|cFF00A2FF"..totalonline)
			...
			if online > 0 or BNonline > 0 then
				...
				if BNonline > 0 then
					...
					for i = 1, BNtotal do
						...
						if client == "WoW" then
							local hasFocus, toonName, client, realmName, realmID, faction, race, class, guild, zoneName, level = BNGetToonInfo(toonID)
							for k,v in pairs(LOCALIZED_CLASS_NAMES_MALE) do if class == v then class = k end end
							if GetLocale() ~= "enUS" then -- feminine class localization (unsure if it's really needed)
								for k,v in pairs(LOCALIZED_CLASS_NAMES_FEMALE) do if class == v then class = k end end
							end
