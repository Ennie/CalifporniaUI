...
if CalifporniaCFG["datatext"].guild and Califpornia.CFG["datatext"].guild > 0 then
	...
	local Text  =Califpornia.Panels.CreateDataText(Califpornia.CFG["datatext"].guild)
	...
	local function Update(self, event, ...)	
		if IsInGuild() then
			...
			Text:SetText(GUILD..": ".."|cFF00A2FF"..numOnline)
		else
	...
	Stat:SetScript("OnEnter", function(self)
		if not InCombatLockdown() then
			if IsInGuild() then
				...
				GameTooltip:AddDoubleLine(GetGuildInfo'player',format("%s %d/%d",GUILD_MEMBERS_ONLINE_COLON,online,total),tthead.r,tthead.g,tthead.b,tthead.r,tthead.g,tthead.b)
				GameTooltip:AddLine' '
...
--						end
	...
	Stat:SetScript("OnMouseDown", function(self, btn) 
		if btn == "LeftButton" then
			...
			-- to directly open guild roster rather than first tab
