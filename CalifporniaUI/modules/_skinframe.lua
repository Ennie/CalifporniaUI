	...
	if addon == "CalifporniaUI" then
		...
		for i = 1, getn(ChatMenus) do
			if _G[ChatMenus[i]] == _G["ChatMenu"] then
				_G[ChatMenus[i]]:HookScript("OnShow", function(self) Califpornia.CreateBD(self) self:ClearAllPoints() self:SetPoint("BOTTOMLEFT", ChatFrame1, "TOPLEFT", 0, 30) end)
			else
				_G[ChatMenus[i]]:HookScript("OnShow", function(self) Califpornia.CreateBD(self) end)
			end
		end

		local shadowskins = {"StaticPopup1", "StaticPopup2", "GameMenuFrame", "InterfaceOptionsFrame", "VideoOptionsFrame", "AudioOptionsFrame", "LFDDungeonReadyStatus", "ChatConfigFrame", "AutoCompleteBox", "SpellBookFrame", "CharacterFrame", "PVPBannerFrame", "PVPFrame", "WorldStateScoreFrame", "GearManagerDialogPopup", "StackSplitFrame", "AddFriendFrame", "FriendsFriendsFrame", "ColorPickerFrame", "ReadyCheckFrame", "PetStableFrame", "LFDDungeonReadyDialog", "ReputationDetailFrame", "LFDRoleCheckPopup"}
-- , "GearManagerDialog" removed in 4.1
		for i = 1, getn(shadowskins) do
			if _G[shadowskins[i]] then
				Califpornia.CreateBD(_G[shadowskins[i]])
				Califpornia.CreateShadow(_G[shadowskins[i]])
			else
				print ('err'..shadowskins[i])
			end
		end
...
-- GameMenuButtonHelp, PaperDollEquipmentManagerPaneSaveSet, PaperDollEquipmentManagerPaneEquipSet added in 4.1
	...
	elseif addon == "Blizzard_LookingForGuildUI" then
		Califpornia.CreateBD(LookingForGuildFrame)
		Califpornia.CreateShadow(LookingForGuildFrame)
		LookingForGuildFrameTabardBackground:Hide()
		LookingForGuildFrameTabardEmblem:Hide()
		LookingForGuildFrameTabardBorder:Hide()
		Califpornia.CreateBD(GuildFinderRequestMembershipFrame)
		Califpornia.CreateShadow(GuildFinderRequestMembershipFrame)
		Califpornia.SkinUIButton(LookingForGuildRequestButton)
		Califpornia.SkinUIButton(LookingForGuildBrowseButton)
		Califpornia.SkinUIButton(GuildFinderRequestMembershipFrameAcceptButton)
		Califpornia.SkinUIButton(GuildFinderRequestMembershipFrameCancelButton)
