if not Califpornia.CFG["nameplate"].enable == true then return end
...
local barTexture = CalifporniaCFG["media"].normTex
local iconTexture = nil
local raidIcons = Califpornia.CFG.media.raidIcons
local overlayTexture = [=[Interface\Tooltips\Nameplate-Border]=]
local font, fontSize, fontOutline = CalifporniaCFG["media"].font, 10, "OUTLINE"
local select = select
local pixelScale = function (...) return ... end
local backdrop = {
	edgeFile = CalifporniaCFG["media"].glowTex, edgeSize = 3,
	insets = {left = 3, right = 3, top = 3, bottom = 13}
}

local IsValidFrame = function(frame)
    if (frame:GetName() and not frame:GetName():find("NamePlate%d")) then
        return
    end

    overlayRegion = select(3, frame:GetRegions())
...
local CreatePlate = function(frame)
    ...
    local glowRegion, highlightRegion, overlayRegion, nameTextRegion, levelTextRegion, bossIconRegion, raidIconRegion, stateIconRegion = frame:GetRegions()
    ...
    healthBar:SetFrameLevel(healthBar:GetFrameLevel() + 2)
    ...
    raidIconRegion:SetPoint("LEFT", healthBar, "RIGHT", pixelScale(1), 0)
