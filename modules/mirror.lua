if not CalifporniaCFG.mirror.enable then return end
...
do
	...
	local loadPosition = function(self)
		local pos = CalifporniaCFG.mirror.positions[self.type]
		local p1, frame, p2, x, y = unpack(pos)
	...
	function Spawn(type)
		...
		local r, g, b = unpack(CalifporniaCFG.mirror.colors[type])

		local bg = frame:CreateTexture(nil, 'BACKGROUND')
		bg:SetAllPoints(frame)
		bg:SetTexture(CalifporniaCFG.media.normTex)
		bg:SetVertexColor(r * .5, g * .5, b * .5)

		local text = frame:CreateFontString(nil, 'OVERLAY')
		text:SetFont(CalifporniaCFG.media.barfont, CalifporniaCFG.mirror.fontsize)
		...
		frame:SetSize(CalifporniaCFG.mirror.width, CalifporniaCFG.mirror.height)

		frame:SetStatusBarTexture(CalifporniaCFG.media.normTex)
