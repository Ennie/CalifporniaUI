if not CalifporniaCFG["nameplate"].enable == true then return end
...
local updatePlate = function(self)
	...
	local newName = (string.len(oldName) > CalifporniaCFG.nameplate.abbrevn) and string.gsub(oldName, "%s?(.[\128-\191]*)%S+%s", "%1. ") or oldName -- "%s?(.)%S+%s"
...
local createPlate = function(frame)
	...
	newNameRegion:SetShadowOffset(1, -1)
	...
	healthBar.hpBackground:SetTexture(CalifporniaCFG["media"].blank)
	...
	castBar.cbBackground:SetTexture(CalifporniaCFG["media"].blank)
...
--	spellIconRegion.IconBackdrop:SetBackdrop(backdrop)
	...
	raidIconRegion:SetTexture(CalifporniaCFG.media.raidIcons)
